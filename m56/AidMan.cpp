...
namespace sdds {
	...
	void AidMan::run() {
		...
		while (returnVal != 0) {
			...
			if (returnVal == 1) {
				...
				cout << endl << "****Ship Items****" << endl << "Shipping Order for ";
				shipping();
				cout<<noOfIprod<< " times saved!" << endl << endl;
				isSave = false;
			}
			else if (returnVal == 7) {
	...
	void AidMan::shipping() {
		ofstream ofstr("shippingOrder.txt");
		ofstream offstr(fileName);
		ofstr << "Shipping Order, Date: ";
		int year, month, day, count = 1, index = 0;
		ut.getSystemDate(&year, &month, &day);
		ofstr << year << '/';
		ofstr.fill('0');
		ofstr.width(2);
		...
		ofstr.width(2);
		ofstr << day << endl;
		ofstr << " ROW |  SKU  | Description                         | Have | Need |  Price  | Expiry" << endl << "-----+-------+-------------------------------------+------+------+---------+-----------" << endl;
		for (int i = 0; i < noOfIprod; i++) {
			if (iProductArray[i]->qty() == iProductArray[i]->qtyNeeded()) {
				ofstr.fill(' ');
				ofstr << " " << fixed << right << setw(3) << count << " | ";
				iProductArray[i]->linear(true);
				iProductArray[i]->display(ofstr);
				ofstr << endl;
				count++;
				remove(i);
			}
	...
	//	noOfIprod -= count;
		for (int i = 0; i < noOfIprod; i++) {
			if (iProductArray[i]->qty() != iProductArray[i]->qtyNeeded()) {
		iProductArray[i]->save(offstr);
		iProductArray[index] = iProductArray[i];
		index++;
			}
		}
		ofstr << "-----+-------+-------------------------------------+------+------+---------+-----------" << endl;
