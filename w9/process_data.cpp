...
namespace sdds_ws9 {
	...
	ProcessData::ProcessData(std::string filename, int n_threads) {
		...
		int firstByte, i=0;
		input_file.read(reinterpret_cast<char*>(&firstByte), sizeof(int));
		total_items = firstByte;
		data = new int[total_items];
		while (!input_file.eof())
		{

			input_file.read(reinterpret_cast<char*>(&data[i]), sizeof(data[i]));
			i++;
		}
	...
	int ProcessData::operator()(std::string fileName, double& average, double& variance) {
		...
		std::vector<std::thread> averageThreads;
		std::vector<std::thread> variationThreads;
		for (size_t i = 0; i < num_threads; i++)
		{
			auto avg = std::bind(computeAvgFactor, &data[p_indices[i]], total_items / num_threads, total_items, ref(averages[i]));;
			averageThreads.push_back(std::thread(avg));
			averageThreads[i].join();
			average += averages[i];
		}
		for (size_t i = 0; i < num_threads; i++)
		{
			auto var = std::bind(computeVarFactor, &data[p_indices[i]], total_items / num_threads, total_items, average, ref(variances[i]));
			variationThreads.push_back(std::thread(var));
			variationThreads[i].join();
